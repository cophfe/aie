...
class Program
{
	static void Main(string[] args)
	{
		...
		while (working)
		{
			...
			//source bitmap image
			Bitmap _bmp;
			string[] files = Directory.GetFiles(@"C:\Users\s210575\Documents\CoPHFe Files\Introduction to C#\TestGamePleaseIgnore\Images\");
			...
			try
			{
				...
				_bmp = new Bitmap(@"C:\Users\s210575\Documents\CoPHFe Files\Introduction to C#\TestGamePleaseIgnore\Images\" + Selector.Select(files));
				Console.Clear();
			}
			catch
			{
		...
		while (happy)
			{
				...
				while (scaling)
				{
					...
					if (_bmp.Width * 2 / (_scaleAmt + 1) < Console.LargestWindowWidth-10)
					{
				...
				//ConsoleColor[,] _img = new ConsoleColor[_bmp.Width, _bmp.Height];
				for (y = 0; y < _bmp.Height - _scaleAmt; y++)
				{
					for (x = 0; x < _bmp.Width-_scaleAmt; x++)
					{
						x += _scaleAmt;
