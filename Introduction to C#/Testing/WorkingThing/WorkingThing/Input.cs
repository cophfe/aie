...
namespace WorkingThing
{
    ...
    class Input
    {
        ...
        static extern IntPtr GetStdHandle( //gets the standard handle
            int nStdHandle
            );

        //CERTIFIED BROKEN LMAO
        ...
        public static void Read()
        {
            ...
            IntPtr h = GetStdHandle(-10); //is correct for some reason
            SetConsoleMode(h, 0x0080); //IS WORKING (TESTED, ACTUALLY FUNCTIONING SOMEHOW)
            SetConsoleMode(h, 0x0008 | 0x0010);
            InputRecord iR = new InputRecord(); // needs to be 128 i think
            uint i = 0;
            Vector2 coord;
            ...
            while (true)
            {
                try
                {
                    ReadConsoleInput(h, ref iR, 128, ref i);
                ...
                Console.WriteLine(iR.Event.KeyEvent.bKeyDown);
                //coord = new Vector2(iR.Event.MouseEvent.dwMousePosition.X, iR.Event.MouseEvent.dwMousePosition.Y);
